...
type ArchiveHomeroomParams struct {
...
type ArchiveHomeroomStmt struct {
	conn      *sqlite.Conn
	stmt      *sqlite.Stmt
	querySQL  string
	hasSlices bool
...
func ArchiveHomeroom(tx *sqlite.Conn) *ArchiveHomeroomStmt {
	const querySQL = `
UPDATE homerooms
SET
...
func (ps *ArchiveHomeroomStmt) Run(
	params ArchiveHomeroomParams,
) (
	err error,
) {
	...
	if _, err := stmt.Step(); err != nil {
		return fmt.Errorf("failed to execute archivehomeroom SQL: %w", err)
	}

	return nil
}

func OnceArchiveHomeroom(
	tx *sqlite.Conn,
	params ArchiveHomeroomParams,
) (
	err error,
) {
	ps := ArchiveHomeroom(tx)
