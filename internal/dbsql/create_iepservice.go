...
type CreateIepserviceParams struct {
	Id              string `json:"id"`
...
type CreateIepserviceStmt struct {
	conn      *sqlite.Conn
	stmt      *sqlite.Stmt
	querySQL  string
	hasSlices bool
...
func CreateIepservice(tx *sqlite.Conn) *CreateIepserviceStmt {
	const querySQL = `
...
func (ps *CreateIepserviceStmt) Run(
	params CreateIepserviceParams,
) (
	err error,
) {
	...
	if _, err := stmt.Step(); err != nil {
		return fmt.Errorf("failed to execute createiepservice SQL: %w", err)
	}

	return nil
}

func OnceCreateIepservice(
	tx *sqlite.Conn,
	params CreateIepserviceParams,
) (
	err error,
) {
	ps := CreateIepservice(tx)
