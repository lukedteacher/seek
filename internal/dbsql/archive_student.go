...
type ArchiveStudentStmt struct {
	conn      *sqlite.Conn
	stmt      *sqlite.Stmt
	querySQL  string
	hasSlices bool
...
func ArchiveStudent(tx *sqlite.Conn) *ArchiveStudentStmt {
	const querySQL = `
UPDATE students
SET
	updated_at = ?1,
...
func (ps *ArchiveStudentStmt) Run(
	params ArchiveStudentParams,
) (
	err error,
) {
	...
	if _, err := stmt.Step(); err != nil {
		return fmt.Errorf("failed to execute archivestudent SQL: %w", err)
	}

	return nil
}

func OnceArchiveStudent(
	tx *sqlite.Conn,
	params ArchiveStudentParams,
) (
	err error,
) {
	ps := ArchiveStudent(tx)
