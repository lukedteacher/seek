...
type ListStudentsWithIepsRes struct {
	Id            string `json:"id"`
...
type ListStudentsWithIepsStmt struct {
	conn      *sqlite.Conn
	stmt      *sqlite.Stmt
	querySQL  string
	hasSlices bool
...
func ListStudentsWithIeps(tx *sqlite.Conn) *ListStudentsWithIepsStmt {
	const querySQL = `
SELECT
	s.id,
	s.marss_id,
...
func (ps *ListStudentsWithIepsStmt) Run() (
	res []ListStudentsWithIepsRes,
	err error,
) {
...
func OnceListStudentsWithIeps(
	tx *sqlite.Conn,
) (
	res []ListStudentsWithIepsRes,
	err error,
) {
	ps := ListStudentsWithIeps(tx)
