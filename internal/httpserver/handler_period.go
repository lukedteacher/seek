...
func (s Server) validateEditPeriod(w http.ResponseWriter, r *http.Request) {
	...
	signals.ID = chi.URLParam(r, "id")
	daysBitmask := models.DaysSignalsToDaysBitmask(signals.Days)
	...
	validation := period.Validate(model)
	patchTempl(w, r, pages.EditPeriod(*signals, *signals, validation))
}
